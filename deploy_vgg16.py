"""
Deploy Pre-Trained TensorFlow Lite VGG16
========================================
...
model_path = './vgg16_uint8_tf2.3_20200929.tflite'
input_name = 'input_1'
...
def get_tvm_accuracy(graph, lib, params, ctx, img_name):
    ...
    image_data = Image.open(img_path+img_name).resize(image_shape).convert('RGB')
    image_data = np.array(image_data).astype(data_type)
    image_data = np.reshape(image_data, data_shape)
